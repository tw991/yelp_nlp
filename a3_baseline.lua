...
function main()
    ...
    model:add(nn.SpatialConvolution(1, 100, 3, 50, 1,1))
    model:add(nn.SpatialMaxPooling(2, 1, 2, 1))
    model:add(nn.SpatialConvolution(100,100, 4, 1, 1, 1))
    model:add(nn.SpatialMaxPooling(2,1,2,1))
    model:add(nn.Reshape(100*23, true))

    model:add(nn.Linear(100*23, 256))
    model:add(nn.ReLU())
